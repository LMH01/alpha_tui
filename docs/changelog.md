...
- Improved order of command line options in `.\alpha_tui load --help` (fr #54)
- Added short forms `-i` for `--index-memory-cells` and `-g` for `--enable-gamma-accumulator` (fr #56)
